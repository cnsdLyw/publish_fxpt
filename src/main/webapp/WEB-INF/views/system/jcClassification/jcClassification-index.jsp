...
<body class="no-skin" >
								...
								<div class="widget-box widget-color-blue2">
									...
									<div class="widget-body" id="test">
										<div class="widget-main padding-8">
											<iframe src="${ctx}/jcClassification/leftTree?code=${code}" name="classTreeFrame" id="classTreeFrame" scrolling="yes" frameborder="0" height="725px" width="100%"></iframe>
										</div>
									</div>
								</div>
							</div>
	...
	<script type="text/javascript">
		...
		function addClass(classKey){
			$("#main_content_configure").attr("src","${ctx}/jcClassification/addClass?classKey="+classKey);
		}
		
		function changeType(classKey){
			$("#classTreeFrame").attr("src","${ctx}/jcClassification/leftTree?classKey="+classKey);
		}
		...
		function deleteClass(classCode){
			...
			bootbox.confirm("确认删除，删除后不可恢复!", function (result) {  
               if(result) {//确认删除  
               		 $.ajax({
					      type : "get",  
					      url : "${ctx}/jcClassification/deleteClass",  
					      dataType:"json",  
					      data:{classCode:classCode},
					      async: false,
					      success : function(obj) {
					      	if(obj.status=="1"){
			      				$("#classTreeFrame").attr("src","${ctx}/jcClassification/leftTree?classKey="+obj.classKey+"&classCode="+obj.classCode);
		...
		function setTreeDate(classKey, classCode){
			$("#classTreeFrame").attr("src","${ctx}/jcClassification/leftTree?classKey="+classKey+"&classCode="+classCode);
		}
